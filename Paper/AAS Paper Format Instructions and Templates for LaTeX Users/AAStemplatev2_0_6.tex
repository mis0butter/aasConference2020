...
Space rendezvous missions have been extensively analyzed using various forms of constrained path planning and optimization techniques. The combination of improved hardware and modern control techniques have made proximity operations more accurate and sophisticated. 

Major breakthroughs in control theory have occurred in the field of robotics and chemical process industries since the 1980's, while the harsh environment of space has limited the kind of hardware, processing power, and control techniques used by spacecraft. Innovations in space-certified sensors and hardware are beginning to expand the capability of spacecraft control and the scope of on-orbit services, such as spacecraft refueling and spacecraft repair. 

Richards and How explored using MPC for RPO[citation]. more previous literature ... MPC stuff ... state identification stuff ... constraints stuff ... 

Richards and How developed and evaluated an MPC application with rendezvous mission against a traditional glideslope approach.

Singh and Bortolami presented an MPC approach for a docking vehicle during its approach phases. They analyzed seven real-world cases of the space shuttle's standard orbit raising maneuver on its way to the ISS.

Filipe and Tsiotras proposed an adaptive position and attitude controller for satellite proximit operations using dual quaternions. Their controller did not require information about the mass and inertia for the chaser satellite.

Brunton, Proctor, and Kutz implemented a controller using Sparse Identification of Nonlinear Dynamics (SINDy). Using data, they derived governing equations for nonlinear dynamic systems, including forcing and input functions.

Brunton, Proctor, and Kutz (https://www.pnas.org/content/113/15/3932.short) use SINDy to derive the dynamics of oscillators, chaotic Lorenz systems, and fluid vortex shedding behind an obstacle.


Though this paper will be applying new techniques toward an established problem, it can hopefully expand the capability of RPO. 
