...
Control methods that utilize system models, such as MPC, are powerful because of their potential application to nonlinear systems with constraints. Nevertheless, this strength in the control domain can come hand-in-hand with a computational efficiency and data logging requirement weakness. The investigations in References \citenum{KaiserKutz_SINDyMPC} and \citenum{BruntonProctor_SINDYc} show how both of these potential drawbacks can be mitigated with the use of SINDY for system identification. These same methods will be adapted to rendezvous proximity maneuver applications.
